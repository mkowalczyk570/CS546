...
export const stringCheck = (arg) =>{
    if(arg == undefined){
        throw `You must provide a string input for eventName, eventDescription, contactEmail, eventDate, startTime, endTime`;
    }
    else if(typeof arg !== 'string'){
        throw `${arg} must be a string`;
    }
    else if(arg.trim().length == 0){
        throw `eventName, eventDescription, contactEmail, eventDate, startTime, endTime cannot be empty space`;
    }
    return arg.trim();
}
...
export const inputCheck = (
    ...
    if(!eventName|| !eventDescription|| !eventLocation|| !contactEmail|| maxCapacity === undefined ||
      ...
      eventDescription = stringCheck(eventDescription);
      contactEmail = stringCheck(contactEmail);
      eventDate = stringCheck(eventDate);
      ...
      if(!emailCheck.test(contactEmail)){throw 'contactEmail is not a valid email'}
      ...
      numCheck(maxCapacity);
      numCheck(priceOfAdmission);
